...
def test_should_method_shouldIgnore_return_False_when_hole_is_not_open(mocker):
    ...
    isHoleOpenSpy.assert_called_once_with(context['gameWindow']['image'], gameWindowCore.images[context['ng_resolution']]
                                          ['holeOpen'], context['ng_radar']['coordinate'], waypoint['coordinate'])
...
def test_should_do(mocker):
    ...
    getSlotFromCoordinateSpy.assert_called_once_with(
        context['ng_radar']['coordinate'], waypoint['coordinate'])

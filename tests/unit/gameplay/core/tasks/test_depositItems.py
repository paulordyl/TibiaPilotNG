...
def test_onBeforeStart():
    ...
    assert task.tasks[2].backpack == context['ng_backpacks']['main']
    assert task.tasks[2].parentTask == task
    assert task.tasks[2].rootTask == task
    assert isinstance(task.tasks[3], ScrollToItemTask)
    assert np.array_equal(
        task.tasks[3].containerImage, images['containersBars'][context['ng_backpacks']['main']]) == True
    assert np.array_equal(
        task.tasks[3].itemImage, images['slots'][context['ng_backpacks']['loot']]) == True
    ...
    assert task.tasks[4].backpack == context['ng_backpacks']['loot']
    ...
    assert task.tasks[6].backpack == context['ng_backpacks']['loot']
    ...
    assert np.array_equal(task.tasks[7].containerBarImage,
                          images['containersBars'][context['ng_backpacks']['loot']]) == True
    assert np.array_equal(

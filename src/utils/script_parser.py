...
VALID_DIRECTIONS = ["north", "south", "east", "west", "center"] # Added center as it's a valid radio option
...
def validate_script_content(script_content_str: str) -> List[str]:
    ...
    try:
        ...
        all_script_labels = [wp.get('label') for wp in waypoints if isinstance(wp, dict) and isinstance(wp.get('label'), str) and wp.get('label')]
    except json.JSONDecodeError as e:
        errors.append(f"Invalid JSON format: {e}")
        return errors

    for i, waypoint in enumerate(waypoints):
        waypoint_errors = _validate_waypoint_object(i, waypoint, all_script_labels)
        errors.extend(waypoint_errors)

    return errors

def _validate_waypoint_object(index: int, waypoint: Any, all_script_labels: List[str]) -> List[str]:
    ...
    if isinstance(waypoint_type, str) and isinstance(waypoint.get('options'), dict):
        options_errors = _validate_waypoint_options(prefix, waypoint_type, waypoint['options'], all_script_labels)
        errors.extend(options_errors)

    return errors

def _validate_waypoint_options(prefix: str, waypoint_type: str, options: Dict[str, Any], all_script_labels: List[str]) -> List[str]:
...
if __name__ == '__main__':
    # If they MUST be empty, add:
    # elif waypoint_type in [...] and options: errors.append(f"{prefix} 'options' for type '{waypoint_type}' must be empty.")
